...
def run_network_cache_test(network_cache_factory: NetworkCacheFactory):
    ...
    # line disconnection
    cache.disconnect_branch_side1('NHV1_NHV2_1')
    cache.disconnect_branch_side2('NHV1_NHV2_1')
    cache.run_ac_pf()
    branches = cache.get_branches()
    ...
    # line reconnection
    cache.connect_branch_side1('NHV1_NHV2_1', 'NHV1')
    cache.connect_branch_side2('NHV1_NHV2_1', 'NHV2')
    cache.run_ac_pf()
    branches = cache.get_branches()
    expected_branches = pd.DataFrame(
        index=pd.Series(name='id', data=['NHV1_NHV2_1', 'NHV1_NHV2_2', 'NGEN_NHV1', 'NHV2_NLOAD']),
