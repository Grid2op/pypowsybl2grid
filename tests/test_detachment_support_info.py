...
def test_None_False():
    env = create_env(
        check_isolated_and_disconnected_injections=None, allow_detachment=False
    )
    if env is None:
    ...
    try:
        ...
        obs, reward, done, info = env.step(
            env.action_space({"set_bus": {"loads_id": [(0, -1)]}})
        )
        assert done
    finally:
        env.close()
...
def test_None_True():
    env = create_env(
        check_isolated_and_disconnected_injections=None, allow_detachment=True
    )
    if env is None:
        # wrong grid2Op version
        return
...
def test_False_False():
    env = create_env(
        check_isolated_and_disconnected_injections=False, allow_detachment=False
    )
    if env is None:
        # wrong grid2Op version
        return
...
def test_False_True():
    env = create_env(
        check_isolated_and_disconnected_injections=False, allow_detachment=True
    )
    if env is None:
        # wrong grid2Op version
        return
...
def test_True_False():
    env = create_env(
        check_isolated_and_disconnected_injections=True, allow_detachment=False
    )
    if env is None:
        # wrong grid2Op version
        return
...
def test_True_True():
    env = create_env(
        check_isolated_and_disconnected_injections=True, allow_detachment=True
    )
    if env is None:
        # wrong grid2Op version
        return
